...
extension, and retained item-containment lower certificate, including every
non-importing lower replay. The active joint residual-screening package adds
29 frozen verification entry points and four fresh independent audits. Its
141 source bindings cover the portable source inventory exactly. It also reruns the formal two-level nonuniform
...
behind the manuscript's rounded-down 98.8779% revenue guarantee.
...
The exact randomized revenue is a frozen reference integral plus the explicit
radical and logarithmic increments in manuscript Section 6. The reported
`lower_floor` is a rational lower enclosure, not a claim that this rational
number equals the exact revenue. The strict gap below 1/100 is checked in
rational arithmetic. Conditional capacity supports cover arbitrary randomized
menus on their stated residuals; they do not prove global auction optimality.
