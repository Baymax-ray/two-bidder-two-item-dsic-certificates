...
\end{aligned}
\]
For every randomized DSIC/IR competitor with selected allocation $a\le r$,
...
\end{aligned}
\]
For every competitor with $a\le r$,
...
The hinge term excludes nonlinear convex trace changes and additional
lotteries, including scarce allocations below one. Thus the finite menu is
an equality case of a bound over the unrestricted conditional class, not a
without-loss restriction imposed on that class.

\subsection{Complete joint reallocations}\label{sec:joint-mechanism}

The archived reference is the explicit V4.5 mechanism, defined by its finite
menu rules, rational tables and algebraic tests in the source-bound
...
Let $R_{4.5}$ be the exact revenue of the source-bound reference, specified
by the included V4.02 continuous integrals plus
...
$\Delta R_1=(1-3D)\varepsilon-3C\varepsilon^2/2-\varepsilon^3/2$.
