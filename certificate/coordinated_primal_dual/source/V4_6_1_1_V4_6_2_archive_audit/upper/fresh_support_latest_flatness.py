...
record={
 ...
 'source_sha256':{p.relative_to(BASE.parents[2]).as_posix():sha256(p.read_bytes()).hexdigest() for p in (LOWER,ARCH/'manifest.json')},
}
target=HERE/'fresh_support_latest_flatness.json'
if '--write' in sys.argv:
    target.write_text(json.dumps(record,indent=2)+'\n',encoding='utf-8')
else:
    assert json.loads(target.read_text(encoding='utf-8'))==record, 'Fresh support record mismatch'
